...
class Trainer:
    ...
    def __init__(self, config):
        ...
        requested_one_logger = bool(getattr(config, "use_one_logger", True))
        self.use_one_logger = requested_one_logger and (OneLoggerUtils is not None)
        if requested_one_logger and (OneLoggerUtils is None) and self.is_main_process:
            print("[Warning] one_logger_utils is not installed; one_logger is disabled.")
        ...
        elif config.distribution_loss == "dmd_hsa":
            self.model = DMDHSA(config, device=self.device)
        ...
        if hasattr(config, 'adapter') and config.adapter is not None:
            ...
            if base_checkpoint_path:
                ...
                if _gen_sd_key is not None:
                    ...
                    result = self.model.generator.load_state_dict(
                        base_checkpoint[_gen_sd_key], strict=not allow_partial_generator_ckpt)
                    missing = list(getattr(result, "missing_keys", []))
                    unexpected = list(getattr(result, "unexpected_keys", []))
                    if allow_partial_generator_ckpt and self.is_main_process:
                        if missing:
                            print(f"[Checkpoint] Missing generator keys ({len(missing)}): {missing[:12]}")
                        if unexpected:
                            print(f"[Checkpoint] Unexpected generator keys ({len(unexpected)}): {unexpected[:12]}")
                    if any("hsa_attention" in key for key in missing):
                        n_init = init_hsa_parameters(self.model.generator)
                        if self.is_main_process:
                            print(f"[HSA-Train] Initialized HSA parameters on {n_init} attention modules "
                                  f"(absent from the base checkpoint)")
                    if self.is_main_process:
        ...
        if config.mixed_precision:
            self._normalize_module_param_dtype(self.model.generator, torch.bfloat16, "generator")
            self._normalize_module_param_dtype(self.model.real_score, torch.bfloat16, "real_score")
            self._normalize_module_param_dtype(self.model.fake_score, torch.bfloat16, "fake_score")
            self._normalize_module_param_dtype(self.model.text_encoder, torch.bfloat16, "text_encoder")
        ...
        elif self.config.distribution_loss in ("dmd_switch", "dmd_hsa"):
            ...
            if checkpoint_path:
                ...
                if "generator" in checkpoint:
                    ...
                    load_result = self.model.generator.load_state_dict(
                        ...
                        strict=not allow_partial_generator_ckpt,
                    )
                ...
                if resume_optimizer_state:
                    if "generator_optimizer" in checkpoint:
                        try:
                            ...
                            if allow_partial_generator_ckpt:
                                if self.is_main_process:
                                    print(f"[Checkpoint] Skip generator optimizer restore: {exc}")
                            else:
                                raise
                    else:
                        if self.is_main_process:
                    ...
                    if "critic_optimizer" in checkpoint:
                        try:
                            ...
                            if allow_partial_generator_ckpt:
                                if self.is_main_process:
                                    print(f"[Checkpoint] Skip critic optimizer restore: {exc}")
                            else:
                                raise
                    else:
                        if self.is_main_process:
    ...
    def _normalize_module_param_dtype(self, module, target_dtype, module_name):
        """Ensure all floating-point params in one module have a uniform dtype for FSDP flattening."""
        floating_dtypes = {p.dtype for p in module.parameters() if p.is_floating_point()}
        if len(floating_dtypes) <= 1 and (len(floating_dtypes) == 0 or target_dtype in floating_dtypes):
            return

        if self.is_main_process:
            sorted_dtypes = ", ".join(sorted([str(dtype) for dtype in floating_dtypes]))
            print(f"[DType] Normalizing {module_name} floating param dtypes: {sorted_dtypes} -> {target_dtype}")
    ...
    def _configure_hsa_trainable(self):
        ...
        hsa_active = (
            ...
            or self.hsa_distill_only
            or self.hsa_distill_weight > 0.0
        )
        if not hsa_active:
            return

        stats = configure_hsa_trainable(
            self.model.generator, hsa_only=self.hsa_train_params_only,
        )
        if self.is_main_process:
            print(
                f"[HSA-Train] hsa_only={self.hsa_train_params_only}, "
                f"HSA parameters trainable={stats['hsa_trainable']}, frozen={stats['frozen']}, "
                f"gate biases reset={stats['gate_biases_reset']}"
            )
    ...
    def fwdbwd_one_step_streaming(self, train_generator):
        ...
        if train_generator:
            ...
            if collect_distill:
                with collect_distill_losses() as distill_terms:
                    generated_chunk, chunk_info = rollout_grad_chunk()
                if distill_terms:
                    hsa_distill_loss = reduce_distill_losses(distill_terms, device=self.device)
            else:
                generated_chunk, chunk_info = rollout_grad_chunk()
            ...
            try:
                ...
                if scaled_generator_loss.requires_grad:
                    ...
                    print("[HSA-Train] Skipping backward: generator loss has no grad this step "
            ...
            if hsa_distill_loss is not None:
                generator_log_dict["hsa_distill_loss"] = hsa_distill_loss.detach()
    ...
    def train(self):
        ...
        try:
            ...
            if self.is_main_process:
                print(f"[ERROR] Training stopped at step {self.step}: {e}")
