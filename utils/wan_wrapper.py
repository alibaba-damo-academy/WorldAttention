...
def _resolve_wan_model_dir(model_name: str) -> Path:
    """Locate a Wan model directory under the weights root.

    The root is ``models`` relative to the working directory; ``MODELS_DIR`` overrides it.
    """
    root = Path(os.environ.get("MODELS_DIR") or os.environ.get("WAN_MODELS_DIR") or "models")
    candidates = [
        root / model_name,
        root / "Wan-AI" / model_name,
    ]
    candidates.extend(sorted(root.glob(f"*/{model_name}")))
    ...
    raise FileNotFoundError(
        f"Could not find Wan model '{model_name}' under {root}. "
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_name: str = "Wan2.1-T2V-1.3B") -> None:
        super().__init__()
        model_dir = _resolve_wan_model_dir(model_name)
        ...
        self.text_encoder.load_state_dict(
            torch.load(model_dir / "models_t5_umt5-xxl-enc-bf16.pth",
...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def adding_cls_branch(self, atten_dim=1536, num_class=4, time_embed_dim=0) -> None:
        # Layer widths follow Wan2.1-T2V-1.3B.
